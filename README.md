...
Durante o projeto foi realizado as etapas de análise exploratória dos dados, transformação de variáveis, identificação e tratamento de valores ausentes, análise univariada e multivariada.
...
Utilizando o Excel, realizei diversas modificações nos dados, provocando problemas e valores ausentes, para que assim seja possível também trabalhar as técnicas de limpeza e tratamento de dados.
...
A nova coluna 'age', possui 15 valores ausentes(nan), além de 10 valores de idade iguais a 0. Entendendo que não existe cliente bancário com idade igual a 0, estes valores também foram identificados como valores ausentes.
...
* O percentual de valores ausentes da coluna **'age'** é muito baixo, em torno de 0,06%. Analisando o histograma, o boxplot, a média, a moda e a mediana desta variável foi possível perceber uma assimetria nos dados e diversos valores outliers, sendo assim a média não deve ser usada como estratégia para preenchimento dos valores ausentes da coluna 'age'. Além disso, a moda(32) é um valor muito abaixo da média(41) e da mediana(39). Se imputar a moda, estaria reforçando essa informação. Portanto, os valores ausentes da variável 'age' foram preenchidos pela **mediana(39)**.

![image](https://user-images.githubusercontent.com/75815212/223592786-44f1b96d-cf83-4c49-a415-510e289435ab.png)

* Na a coluna **'month'**, que representa o mês do contato com o cliente, possuía 40 valores ausentes, representando um percentual de 0,09%. A variável 'month' é do tipo categórica, portanto, tomei a decisão de preencher seus valores ausentes com a **moda(may)**.
...
Analisando o gráfico de barras abaixo é possível concluir que mais de 45% dos clientes contactados nesta campanha possuíam entre 30 e 41 anos.
...
A correlação é um coeficiente entre -1 e +1, sendo +1 uma alta correlação positiva, -1 uma alta correlação negativa e próximo de zero não há correlação entra as variáveis.
